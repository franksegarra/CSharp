...
namespace NAPowerWebAPI.Models
{
    ...
    {
        private List<UserAccount> _account;
        private int _nextId;
        ...
        {
            return _account.OrderBy( i => i.Id);
        }

        public UserAccount GetUserAccount(int id)
        {
            return _account.Where(p => p.Id == id).FirstOrDefault();
        }
        ...
        public bool Update(UserAccount item)
        {
            ...
            int index = _account.FindIndex(p => p.Id == item.Id);
            if (index == -1)
            {
                return false;
            }
